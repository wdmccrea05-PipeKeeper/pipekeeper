...
// 14. Duplicate subscription guard regression tests (P0: Dallas Hington triplicate billing)
runCheck('14. Duplicate subscription guard regression tests', () => {
  try {
    const out = exec('npx vitest run src/__tests__/duplicateSubscriptionGuard.test.js --reporter=default 2>&1', 120000);
    if (/Test Files.*1 failed/.test(out)) throw new Error('Test failures detected');
    return 'Duplicate subscription guard tests passed';
  } catch (e) {
    const out = e.stdout || e.message;
    throw new Error(out.substring(0, 500));
  }
...
// 15. Contract → Entitlement reconciliation tests (P0: paid but Free bug)
runCheck('15. Contract → Entitlement reconciliation tests', () => {
  try {
    const out = exec('npx vitest run src/__tests__/reconcileEntitlementForUser.test.js --reporter=default 2>&1', 120000);
    if (/Test Files.*1 failed/.test(out)) throw new Error('Test failures detected');
    return 'Contract → Entitlement reconciliation tests passed';
  } catch (e) {
    const out = e.stdout || e.message;
    throw new Error(out.substring(0, 500));
  }
...
// 19. Final entitlement reconciliation invariants (P0: Product ID permanence + eligibility)
runCheck('19. Final entitlement reconciliation invariants', () => {
  try {
    const out = exec('npx vitest run src/__tests__/finalEntitlementReconciliation.test.js --reporter=default 2>&1', 120000);
    if (/Test Files.*1 failed/.test(out)) throw new Error('Test failures detected');
    return 'Final entitlement reconciliation invariants passed';
  } catch (e) {
    const out = e.stdout || e.message;
    throw new Error(out.substring(0, 500));
  }
