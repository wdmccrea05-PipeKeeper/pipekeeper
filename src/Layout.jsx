...
/** Hard-enforced PipeKeeper theme tokens (works even if globals.css is ignored/overridden). */
function applyPipeKeeperTheme() {
  const root = document.documentElement;

  // ensure dark class doesn't fight you
  ...
  // Make absolutely sure the page background flips even if Base44 body styles win
...
export default function Layout({ children, currentPageName }) {
  ...
  // ✅ FORCE THEME ONCE PER LOAD (and again after fast refresh)
  React.useEffect(() => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          />
          <p style={{ color: "hsl(var(--foreground) / 0.80)" }}>Loading...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        ...
        >
          ...
          {isCompanionApp() && (
            <p className="text-sm mb-4" style={{ color: "hsl(var(--foreground) / 0.80)" }}>
              In the companion app, please sign in using your email and password.
            </p>
          )}
          <p className="mb-6" style={{ color: "hsl(var(--foreground) / 0.70)" }}>
  ...
  return (
    <>
      {/* Extra safety: inject tokens into CSS cascade too */}
      <style>{`
        :root, .dark {
          --background: 214 53% 9%;
          --foreground: 40 75% 87%;
      ...
      <TermsGate user={user}>
        <div className="min-h-screen">
          {/* Desktop Navigation */}
          <nav className="hidden md:flex fixed top-0 left-0 right-0 z-50 backdrop-blur-lg border-b shadow"
            ...
            <div className="mx-auto w-full max-w-6xl px-6">
              <div className="flex items-center justify-between h-16 gap-4">
                <Link to={createPageUrl('Home')} className="flex items-center gap-3 flex-shrink-0">
                  ...
                  <span className="font-bold text-xl" style={{ color: "hsl(var(--foreground))" }}>PipeKeeper</span>
                </Link>
          ...
          {/* Mobile Navigation */}
          <nav className="md:hidden fixed top-0 left-0 right-0 z-50 backdrop-blur-lg border-b shadow"
               style={{ background: "hsl(var(--background) / 0.85)", borderColor: "hsl(var(--border) / 0.75)" }}>
            <div className="flex items-center justify-between h-14 px-4">
              <Link to={createPageUrl('Home')} className="flex items-center gap-2" onClick={() => setMobileOpen(false)}>
                ...
                <span className="font-bold text-lg" style={{ color: "hsl(var(--foreground))" }}>PipeKeeper</span>
              </Link>
              <button
                onClick={(e) => {
                  e.stopPropagation();
            ...
            <div className="mx-auto w-full max-w-6xl px-6 py-6">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm" style={{ color: "hsl(var(--foreground) / 0.70)" }}>
                    © 2025 PipeKeeper. All rights reserved.
                  </span>
                </div>
                <div className="flex gap-6">
                  <a href={createPageUrl('FAQ')} className="text-sm hover:underline"
                     style={{ color: "hsl(var(--foreground) / 0.70)" }}>
                    FAQ
                  </a>
                  <a href={createPageUrl('Support')} className="text-sm hover:underline"
                     style={{ color: "hsl(var(--foreground) / 0.70)" }}>
                    Support
                  </a>
                  <a href={createPageUrl('TermsOfService')} className="text-sm hover:underline"
                     style={{ color: "hsl(var(--foreground) / 0.70)" }}>
                    Terms of Service
                  </a>
                  <a href={createPageUrl('PrivacyPolicy')} className="text-sm hover:underline"
                     style={{ color: "hsl(var(--foreground) / 0.70)" }}>
                    Privacy Policy
                  </a>
                </div>
