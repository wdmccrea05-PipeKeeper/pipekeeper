      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-[#F5F1E7]">
            {tasting.rating ? `⭐ ${safePrimitive(tasting.rating)}` : t('common.unratedTasting')}
          </p>
          ...
          <p className="text-sm text-[#E0D8C8]/84 mt-3 break-words whitespace-pre-wrap">
            {safePrimitive(tasting.notes, t('common.noNotes'))}
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr]">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                {avgRating ? (
                  <DetailStat label={t('whiskey.avgRating')} value={`${avgRating} / 5`} icon={Star} />
                ) : null}
                {bottle.age ? (
                  <DetailStat label={t('whiskey.age')} value={`${safePrimitive(bottle.age)} years`} icon={CalendarDays} />
                ) : null}
                {bottle.abv ? (
                  <DetailStat label={t('whiskey.abv')} value={`${safePrimitive(bottle.abv)}%`} icon={Sparkles} />
                ) : null}
                {bottle.bottle_size ? (
                  <DetailStat label={t('whiskey.bottleSize')} value={safePrimitive(bottle.bottle_size)} icon={Package} />
                ) : null}
                {bottle.bottle_count > 1 ? (
                  <DetailStat label={t('whiskey.bottleCount')} value={safePrimitive(bottle.bottle_count)} icon={Package} />
                ) : null}
                {bottle.fill_level ? (
                  <DetailStat label={t('whiskey.fillLevel')} value={safePrimitive(bottle.fill_level)} icon={Package} />
                ) : null}
                {totalValue > 0 && bottle.bottle_count > 1 ? (
                  <DetailStat label={t('hub.totalValue')} value={formatFromBase(totalValue)} icon={WhiskeyKeeperIcon} />
                ) : null}
              </div>
