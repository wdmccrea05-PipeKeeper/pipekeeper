...
export default function PublicSharedRecord() {
  ...
  return <div className="min-h-screen" style={{ background: "radial-gradient(circle at 30% 20%, rgba(120,85,55,0.15), transparent 40%), radial-gradient(circle at 80% 70%, rgba(70,50,35,0.2), transparent 50%), #0f0b08" }}>
    ...
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="text-center max-w-md mx-auto"><div className="bg-gradient-to-br from-[#2a1f18] to-[#1f1510] border border-[rgba(180,140,75,0.25)] rounded-lg p-6"><p style={{ color: "rgba(224, 216, 200, 0.8)", marginBottom: 12, fontSize: 14 }}>{t("share.startOwnCollection", { defaultValue: "Start your own collection" })}</p><Button onClick={() => (window.location.href = "/")} className="w-full bg-[#A35C5C] hover:bg-[#8F4E4E]">{t("share.openPipeKeeper", { defaultValue: "Open PipeKeeper" })}</Button><p style={{ color: "rgba(180, 140, 75, 0.6)", fontSize: 11, marginTop: 12, textTransform: "uppercase", letterSpacing: "0.05em" }}>{t("share.poweredByPipeKeeper", { defaultValue: "Shared from PipeKeeper" })}</p></div></div>
    </div>
  </div>;
}
