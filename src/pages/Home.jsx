...
export default function HomePage() {
  ...
  /**
   * ✅ Premium gating: use Subscription entity as source-of-truth
   * This is what fixes paid users still seeing locked Premium cards.
   */
  const { hasPremium: isPaidUser, isLoading: premiumLoading } = usePremiumAccess(user);
  ...
  const updateOnboardingMutation = useMutation({
    mutationFn: ({ id, data }) =>
      safeUpdate("OnboardingStatus", id, data, user?.email),
  });
  ...
  const getCellarBreakdown = () => {
    ...
    safeCellarLogs.forEach((log) => {
      ...
      if (log.transaction_type === "added") byBlend[log.blend_id].totalOz += log.amount_oz || 0;
      if (log.transaction_type === "removed") byBlend[log.blend_id].totalOz -= log.amount_oz || 0;
    });
            ...
            <div className="p-6">
              <p className="text-stone-700 text-lg leading-relaxed mb-4">
                ...
                <span className="font-semibold text-stone-900">
                  no subscription fees will be charged until after January 15, 2026
                </span>
                .
              </p>
          ...
          <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-8">
                  ...
                  <Card className="bg-gradient-to-br from-[#8b3a3a] to-[#6d2e2e] border-[#e8d5b7]/30 cursor-pointer hover:shadow-lg transition-shadow">
                    <CardContent className="p-3 sm:p-6 text-center">
                      <img
                        src={PIPE_ICON}
                        alt="Pipes"
                        className="w-6 h-6 sm:w-8 sm:h-8 mx-auto mb-1 sm:mb-2 object-contain"
            ...
            {safePipes.length > 0 && safeBlends.length > 0 && user && !pipesLoading && !blendsLoading ? (
              ...
              >
                {isAppleBuild ? (
                  ...
                  />
                ) : premiumLoading ? null : isPaidUser ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 w-full">
              ...
              {recentBlends.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.0 }} className="w-full">
                  <Card className="border-stone-200 w-full">
                    <CardHeader className="flex flex-row items-center justify-between p-4 sm:p-6">
                            ...
                            <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-lg hover:bg-stone-50 transition-colors cursor-pointer">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs sm:text-sm text-stone-500 truncate">
                                  {blend.manufacturer || blend.blend_type || "Unknown"}
                                </p>
                              </div>
            ...
            {safePipes.length === 0 && safeBlends.length === 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="text-center py-8 sm:py-12 px-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center max-w-sm mx-auto">
                  <a href={createPageUrl("Pipes")} className="w-full sm:w-auto">
                    <Button className="bg-amber-700 hover:bg-amber-800 w-full">Add Your First Pipe</Button>
                  </a>
                  <a href={createPageUrl("Tobacco")} className="w-full sm:w-auto">
                    <Button variant="outline" className="w-full">Add Your First Blend</Button>
                  </a>
                </div>
              </motion.div>
            )}
