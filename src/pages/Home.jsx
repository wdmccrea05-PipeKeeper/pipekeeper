...
export default function HomePage() {
    ...
    return (
      <div className="pk-page flex items-center justify-center p-4">
        <div className="text-center">
          ...
          />
          <p className="text-foreground/80">Loading user...</p>
        </div>
      </div>
    );
  }
  ...
  if (userError) {
    ...
    return (
      <div className="pk-page flex items-center justify-center p-4">
        <Card className="max-w-md w-full pk-card">
          <CardContent className="p-8 text-center">
            <AlertCircle className="w-12 h-12 text-destructive mx-auto mb-4" />
            <h2 className="text-xl font-bold text-foreground mb-2">Login Required</h2>
            <p className="text-foreground/70 mb-4">Please log in to continue</p>
      ...
      <div className="pk-page flex items-center justify-center p-4">
        <div className="text-center">
          <img
            ...
            className="w-32 h-32 mx-auto mb-4 object-contain animate-pulse"
          />
          <p className="text-foreground/80">Loading your collection...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="pk-page flex items-center justify-center p-4">
        <Card className="max-w-md w-full pk-card">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-foreground/70 mb-4">Please refresh to try again</p>
  ...
  return (
    <>
      ...
      {showTestingNotice && !showOnboarding ? (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <motion.div
            ...
            className="pk-card max-w-lg w-full overflow-hidden"
          >
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm opacity-90 mt-1">Important Information</p>
                  </div>
                </div>
                <button
            ...
            <div className="p-6 bg-card text-card-foreground">
              <p className="text-lg leading-relaxed mb-4">
                During testing, <span className="font-semibold">no subscription fees will be charged until after January 15, 2026</span>.
              </p>
              <p className="text-sm text-foreground/70">
                All premium features are available free of charge during this period. Thank you for helping us test PipeKeeper!
              </p>
              <Button onClick={handleDismissNotice} className="w-full mt-6">
                Got it, thanks!
              </Button>
            </div>
          </motion.div>
      ...
      <div className="pk-page overflow-x-hidden">
        <div className="pk-shell py-6 sm:py-8">
          {/* Hero */}
          <motion.div
              ...
              <a href={createPageUrl('Pipes')}>
                <Card className="bg-gradient-to-br from-accent to-accent/80 border-border/60 cursor-pointer hover:shadow-lg transition-shadow">
                  <CardContent className="p-3 sm:p-6 text-center">
                    <PipeKeeperPipeIcon className="w-7 h-7 sm:w-9 sm:h-9 mx-auto mb-1 sm:mb-2 text-primary" strokeWidth={2.2} />
                    <p className="text-2xl sm:text-3xl font-bold text-primary">{safePipes.length}</p>
                    <p className="text-xs sm:text-sm text-primary/70">Pipes</p>
                  </CardContent>
                </Card>
              </a>
            </motion.div>
          ...
          <div className="grid md:grid-cols-2 gap-4 sm:gap-6 mb-8 sm:mb-12">
            ...
            >
              <a href={createPageUrl('Pipes')}>
                <Card className="h-full hover:shadow-xl transition-all border-primary/20 overflow-hidden cursor-pointer group relative">
                  <div className="absolute inset-0 bg-gradient-to-br from-accent/70 via-accent/55 to-accent/70 z-10" />
                  <div
                  ...
                  <div className="relative z-20 p-4 sm:p-8 h-full flex flex-col">
                    ...
                    <div className="flex-1 mb-4 sm:mb-6">
                      ...
                      <ul className="space-y-2 sm:space-y-2.5 text-primary text-sm sm:text-base">
            ...
            >
              <a href={createPageUrl('Tobacco')}>
                <Card className="h-full hover:shadow-xl transition-all border-primary/20 overflow-hidden cursor-pointer group relative">
                  <div className="absolute inset-0 bg-gradient-to-br from-secondary/70 via-secondary/55 to-secondary/70 z-10" />
                  <div
                    className="absolute inset-0 bg-cover bg-center opacity-35 group-hover:opacity-45 transition-opacity"
                    style={{
                  ...
                  <div className="relative z-20 p-4 sm:p-8 h-full flex flex-col">
                    <div className="flex items-start justify-between mb-4 sm:mb-6">
                      <div className="flex-1 min-w-0">
                        <h3 className="text-2xl sm:text-3xl font-bold text-primary mb-1 sm:mb-2">Tobacco Cellar</h3>
                        <p className="text-primary text-sm sm:text-base">Manage your blends</p>
                      </div>
                      <div className="bg-white/10 backdrop-blur-sm rounded-lg p-2 sm:p-3 flex-shrink-0 ml-2">
                        <TobaccoLeafIcon className="w-6 h-6 sm:w-8 sm:h-8 text-primary" strokeWidth={2.0} />
                      </div>
                    </div>

                    <div className="flex-1 mb-4 sm:mb-6">
                      <div className="bg-black/20 backdrop-blur-sm rounded-lg p-3 sm:p-4 mb-2 sm:mb-3">
                        <p className="text-3xl sm:text-4xl font-bold text-primary mb-1">{safeBlends.length}</p>
                        <p className="text-primary text-sm sm:text-base">Tobacco Blends</p>
                      </div>
          ...
          {(favoritePipes.length > 0 || favoriteBlends.length > 0) && (
            <motion.div
              ...
              className="mb-12"
            >
              <Card className="pk-card bg-card/70">
                <CardHeader>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {favoritePipes.map(pipe => (
                      <a key={pipe.id} href={createPageUrl(`PipeDetail?id=${encodeURIComponent(pipe.id)}`)}>
                        <Badge className="bg-secondary/70 text-foreground border-border hover:bg-secondary cursor-pointer px-3 py-1.5">
                          🪈 {pipe.name}
                        </Badge>
                      </a>
                    ))}
                    {favoriteBlends.map(blend => (
                      <a key={blend.id} href={createPageUrl(`TobaccoDetail?id=${encodeURIComponent(blend.id)}`)}>
                        <Badge className="bg-secondary/40 text-foreground border-border hover:bg-secondary/70 cursor-pointer px-3 py-1.5">
                          🍂 {blend.name}
                        </Badge>
                      </a>
                    ))}
          ...
          >
            <a href={createPageUrl('Import')}>
              <Card className="border-primary/25 bg-card/60 hover:shadow-lg transition-shadow cursor-pointer">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-foreground">Bulk Import</p>
                        <p className="text-xs text-foreground/70">Import pipes & tobacco from CSV</p>
                      </div>
                    </div>
              ...
              >
                <Card className="pk-card w-full bg-card/70">
                  <CardHeader className="flex flex-row items-center justify-between p-4 sm:p-6">
                    <CardTitle className="text-primary text-lg">Recent Pipes</CardTitle>
                    <a href={createPageUrl('Pipes')}>
                  ...
                  <CardContent className="p-4 sm:p-6">
                    <div className="space-y-2 sm:space-y-3">
                      {recentPipes.map(pipe => (
                        <a key={pipe.id} href={createPageUrl(`PipeDetail?id=${encodeURIComponent(pipe.id)}`)}>
                          <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-xl hover:bg-secondary/40 transition-colors cursor-pointer">
                            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-secondary/60 overflow-hidden flex items-center justify-center flex-shrink-0 border border-border">
                              {pipe.photos?.[0] ? (
                  ...
                  <CardContent className="p-4 sm:p-6">
                    <div className="space-y-2 sm:space-y-3">
                      {recentBlends.map(blend => (
                        <a key={blend.id} href={createPageUrl(`TobaccoDetail?id=${encodeURIComponent(blend.id)}`)}>
                          <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-xl hover:bg-secondary/40 transition-colors cursor-pointer">
                            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-secondary/50 overflow-hidden flex items-center justify-center flex-shrink-0 border border-border">
          ...
          {safePipes.length === 0 && safeBlends.length === 0 && (
            ...
            >
              ...
              <p className="text-sm sm:text-base text-foreground/70 mb-6 sm:mb-8 max-w-md mx-auto px-2">
