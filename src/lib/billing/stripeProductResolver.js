/**
 * JS mirror of base44/shared/stripeProductResolver.ts for test compatibility.
 * This file MUST stay in sync with the TypeScript shared module.
 */
...
export function mapProductNameToPlan(name) {
  if (!name) return null;
  const lower = name.toLowerCase();
  const isAnnual =
    lower.includes('annual') || lower.includes('yearly') || lower.includes('year');
  const isMonthly =
...
export function extractStripeChainData(stripeSubscription) {
  ...
  return {
    ...
    product_active:
      productObj && typeof productObj.active === 'boolean' ? productObj.active : null,
    billing_interval: normalizeInterval(price.recurring?.interval || null),
    amount_cents: typeof price.unit_amount === 'number' ? price.unit_amount : null,
    currency: price.currency || null,
  };
}

export function buildRegistryEntryFromStripe(chain, planKey, mappingSource, confidence) {
...
export function resolveProductIdentityFromStripeChain(input) {
  ...
  if (contract.provider === 'apple') {
    const appleProductId =
      contract.resolved_product_id || legacy_subscription?.product_id || null;
  ...
  // 10. AMOUNT INFERENCE (last resort)
...
export function detectProviderMismatch(contract, resolverResult) {
...
export function classifyUserPopulation(userId, contracts) {
