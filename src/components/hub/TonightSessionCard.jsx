...
import { Sparkles, Moon, RefreshCw, ChevronRight } from 'lucide-react';
...
export default function TonightSessionCard({ pipes = [], blends = [], bottles = [], profile = null }) {
  ...
  async function generateRecommendation(force = false) {
    ...
    const pipesList = pipes.slice(0, 20).map(p =>
      `${p.name} (${p.maker || 'unknown'}, ${p.shape || 'unknown shape'}, bowl: ${p.sizeClass || 'standard'}${p.is_favorite ? ', favorite' : ''})`
    ).join('\n');

    const blendsList = blends.slice(0, 20).map(b =>
      `${b.name} (${b.manufacturer || 'unknown'}, ${b.blend_type || 'unknown type'}, strength: ${b.strength || 'unknown'})`
    ).join('\n');

    const bottlesList = bottles.slice(0, 10).map(b =>
      `${b.name} (${b.distillery || 'unknown'}, ${b.whiskey_type || 'unknown type'}, ${b.age_years ? b.age_years + ' yr' : 'NAS'})`
    ).join('\n');

    const whiskyPrefs = profile?.whiskey_preferences;
    const prefSummary = whiskyPrefs
      ? `Types: ${(whiskyPrefs.types || []).join(', ') || 'any'}; Flavors: ${(whiskyPrefs.flavors || []).join(', ') || 'any'}; Style: ${(whiskyPrefs.drinking_style || []).join(', ') || 'any'}`
      : 'No whiskey preferences set';

    const tobaccoPrefs = profile
      ? `Preferred blends: ${(profile.preferred_blend_types || []).join(', ') || 'any'}; Strength: ${profile.strength_preference || 'any'}`
      : 'No tobacco preferences set';

    const prompt = `You are a personal collector advisor. Based on this collection, generate a "Tonight's Session" recommendation.
...
1. pipe (pick from the list above)
2. blend (pick from the list above) 
3. whiskey (pick from bottles above, or omit if none)
4. flavor_theme (2-4 word evocative theme)
5. rationale (1-2 sentences explaining why this combination works)

Return JSON with keys: pipe, blend, whiskey, flavor_theme, rationale`;
