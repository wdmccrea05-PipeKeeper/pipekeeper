...
export default function TonightSessionCard({
  ...
  async function recordSession() {
    if (!recommendation?.pipe_id || !recommendation?.blend_id) {
      toast.error(t("session.invalidData"));
      return;
    }
    ...
    try {
      ...
      toast.success(t("session.recorded"));
    } catch (error) {
      console.error("[TonightSessionCard] record failed:", error);
      toast.error(t("session.recordFailed"));
    } finally {
        ...
        {loading && !current ? (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {current.pipe ? (
                <RecommendationSlot label={t("common.pipe")} value={current.pipe} accent="#D4A574" />
              ) : null}
              {current.blend ? (
                <RecommendationSlot label={t("common.tobacco")} value={current.blend} accent="#7AAA68" />
              ) : null}
              {current.whiskey ? (
                <RecommendationSlot label={t("common.whiskey")} value={current.whiskey} accent="#74A5D4" />
              ) : null}
            </div>
