...
async function enrichStoryHighlights(story) {
  ...
  const nextHighlights = { ...h };

  const [
    mostUsedPipeRecord,
    favoritePipeRecord,
    favoriteBlendRecord,
    favoriteBottleRecord,
    mostTastedBottleRecord,
  ] = await Promise.all([
    h.mostUsedPipe?.id ? safeGet(base44.entities.Pipe, h.mostUsedPipe.id) : null,
    h.favoritePipe?.id ? safeGet(base44.entities.Pipe, h.favoritePipe.id) : null,
    h.favoriteBlend?.id ? safeGet(base44.entities.TobaccoBlend, h.favoriteBlend.id) : null,
    h.favoriteBottle?.id ? safeGet(base44.entities.Bottle, h.favoriteBottle.id) : null,
    h.mostTastedBottle?.id ? safeGet(base44.entities.Bottle, h.mostTastedBottle.id) : null,
  ]);

  if (h.mostUsedPipe) {
    nextHighlights.mostUsedPipe = {
      ...h.mostUsedPipe,
      _record: mostUsedPipeRecord,
      recordType: 'pipe',
    };
  }

  if (h.favoritePipe) {
    nextHighlights.favoritePipe = {
      ...h.favoritePipe,
      _record: favoritePipeRecord,
      recordType: 'pipe',
    };
  }

  if (h.favoriteBlend) {
    nextHighlights.favoriteBlend = {
      ...h.favoriteBlend,
      _record: favoriteBlendRecord,
      recordType: 'blend',
    };
  }

  if (h.favoriteBottle) {
    nextHighlights.favoriteBottle = {
      ...h.favoriteBottle,
      _record: favoriteBottleRecord,
      recordType: 'bottle',
    };
  }

  if (h.mostTastedBottle) {
    nextHighlights.mostTastedBottle = {
      ...h.mostTastedBottle,
      _record: mostTastedBottleRecord,
      recordType: 'bottle',
    };
  }

  if (h.mostValuableItem?.id) {
    let record = null;
    let recordType = h.mostValuableItem.type || null;

    if (recordType === 'pipe') {
      record = await safeGet(base44.entities.Pipe, h.mostValuableItem.id);
    } else if (recordType === 'blend') {
      record = await safeGet(base44.entities.TobaccoBlend, h.mostValuableItem.id);
    } else if (recordType === 'bottle') {
      record = await safeGet(base44.entities.Bottle, h.mostValuableItem.id);
    } else {
      record =
        (await safeGet(base44.entities.Pipe, h.mostValuableItem.id)) ||
        (await safeGet(base44.entities.TobaccoBlend, h.mostValuableItem.id)) ||
        (await safeGet(base44.entities.Bottle, h.mostValuableItem.id));

      if (record) {
        if (record.photo) recordType = 'bottle';
        else if (record.logo || record.photo) recordType = 'blend';
        else recordType = 'pipe';
      }
    }

    nextHighlights.mostValuableItem = {
      ...h.mostValuableItem,
      _record: record,
      recordType,
    };
