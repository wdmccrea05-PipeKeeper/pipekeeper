    ...
    faqFull: {
      ...
      sections: {
        general: { title: "一般", items: [
          { id: "what-is", q: "PipeKeeperとは何ですか?", a: "PipeKeeperはパイプ喫煙者向けに設計されたコレクション管理および情報アプリケーションです。パイプ、タバコブレンド、熟成缶、関連メモを追跡し、オプションのAI支援インサイト および価値評価を提供します。" },
        ...
        accountsAndData: { title: "アカウントとデータ", items: [
          ...
          { id: "bulk-import", q: "データを一括インポートできますか?", a: "はい。ホーム画面からインポートページに移動します。CSVデータを貼り付けるか、ファイルをアップロードして、複数のパイプまたはタバコブレンドを一度に素早く追加できます。" }
        ]},
    ...
    howTo: {
      ...
      sections: {
        ...
        aiTools: { title: "AIツール", items: [
          ...
          { id: "optimize-collection", q: "コレクションを最適化するにはどうしたらいいですか?", path: "ホーム → タバコエキスパート → オプティマイズ", badge: "Pro", a: "コレクション最適化ツールはパイプとタバコを分析して、専門化を提案し、ギャップを特定し、次の購入を提案します。" }
        ]},
    ...
    faqFull: {
      ...
      sections: {
        ...
        featuresAndTools: { title: "功能和工具", items: [
          { id: "interchangeable-bowls", q: "什么是可互换的碗?", intro: "某些烟斗系统(Falcon、Gabotherm、Yello-Bole、Viking等)允许您在同一杆/腔室组件上交换不同的碗。PipeKeeper将每个碗视为具有其自身的不同'烟斗变体':", points: ["焦点标签(将一个碗专用于弗吉尼亚烟,另一个专用于香料烟等)", "腔室尺寸和特性", "烟草配对建议", "磨合时间表和吸烟日志"], conclusion: "这可实现最优专业化:将同一杆与多个碗一起用于不同的烟草类型,而无需幽灵。" },
