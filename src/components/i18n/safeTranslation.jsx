...
export function useTranslation(langOverride) {
  const lang = useMemo(() => {
    ...
    try {
      const stored = window?.localStorage?.getItem("pk_lang") || window?.localStorage?.getItem("pipekeeper.language") || window?.localStorage?.getItem("pk.language") || window?.localStorage?.getItem("language");
