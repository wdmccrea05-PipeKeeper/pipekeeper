...
export default function LogSessionModal({ isOpen, onClose, pipes = [], blends = [], user }) {
  ...
  const { data: recentLogs = [] } = useQuery({
    ...
    queryFn: () =>
      base44.entities.SmokingLog.filter({ created_by: user?.email }, "-date", 50),
    enabled: !!user?.email,
    staleTime: 30000,
  });
  ...
  const getPipeRestStatus = (pipeId) => {
    const pipeLogs = (recentLogs || [])
      ...
      .sort((a, b) => {
        try {
          return (
            parseLocalCalendarDate(b.date).getTime() -
            parseLocalCalendarDate(a.date).getTime()
          );
        } catch {
          return 0;
        }
    ...
    try {
      ...
      if (Number.isNaN(lastSmoked.getTime()))
      ...
      return {
        ready: false,
        message: t("smokingLog.needsHours", { hours: hoursLeft.toFixed(1) }),
      };
    } catch {
      return { ready: true, message: t("smokingLog.noUsageLogged") };
    }
  ...
  const createLogMutation = useMutation({
    ...
    onSuccess: async (_, variables) => {
      if (variables.container_id) {
        try {
          const containerRes = await base44.entities.TobaccoContainer.filter({
          ...
          if (container?.id) {
            ...
            await safeUpdate(
              ...
              container.id,
              {
                quantity_grams: Math.max(
                  0,
                  Number(container.quantity_grams || 0) - gramsUsed
                ),
            ...
            queryClient.invalidateQueries({
              queryKey: ["containers", user?.email, variables.blend_id],
            });
          }
        } catch (err) {
      ...
      if (variables.is_break_in && variables.pipe_id && variables.blend_id) {
        ...
        if (pipe?.id) {
          ...
          const schedule = Array.isArray(pipe.break_in_schedule)
            ? pipe.break_in_schedule
            : [];
          ...
          await safeUpdate(
            "Pipe",
            pipe.id,
            { break_in_schedule: updatedSchedule },
            user?.email
          );
  ...
  const handleSubmit = (e) => {
    ...
    if (entitlements.tier === "free") {
      if ((recentLogs || []).length >= entitlements.limits.smokingLogs) {
        toast.error(
          t("smokingLog.freeLimitReached", { limit: entitlements.limits.smokingLogs })
        );
        return;
      }
    }
    ...
    if (formData.bowl_variant_id && hasMultipleBowls) {
      const bowl = selectedPipe.interchangeable_bowls.find(
        (b) =>
          (b.bowl_variant_id ||
            `bowl_${selectedPipe.interchangeable_bowls.indexOf(b)}`) ===
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v) =>
                setFormData({ ...formData, blend_id: v, container_id: "" })
              }
            >
              <SelectTrigger>
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button
              type="submit"
              disabled={
                !formData.pipe_id || !formData.blend_id || createLogMutation.isPending
              }
              className="flex-1"
            >
