...
export default function LogSessionModal({ isOpen, onClose, pipes = [], blends = [], user, initialPipeId = "", initialBlendId = "" }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (saving || postPromptItems || postPromptPendingRef.current) return; // prevent duplicate saves and re-saves while post-prompt is shown
    ...
    try {
      ...
      const logData = prepareLogData({
        ...
        ...(pipeMode === "collection" && formData.bowl_variant_id ? { bowl_variant_id: formData.bowl_variant_id } : {}),
        ...(blendMode === "collection" && formData.container_id ? { container_id: formData.container_id } : {}),
        pipe_name,
        blend_name,
        bowl_name,
