...
import { Cigarette, DollarSign, BookOpen, Heart, Clock } from 'lucide-react';
...
export default function CigarInsights({ cigars = [], sessions = [], humidors = [] }) {
  ...
  // Aging status
  const agingCigars = cigars.filter((c) => c.aging_start_date);
  const readyNow = agingCigars.filter((c) => !c.ready_to_smoke_date || new Date(c.ready_to_smoke_date) <= today);
  const notReady = agingCigars.filter((c) => c.ready_to_smoke_date && new Date(c.ready_to_smoke_date) > today);
      ...
      {agingCigars.length > 0 && (
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              <div className="text-2xl font-bold text-[#F5F1E7]">{readyNow.length}</div>
            ...
            >
              <div className="text-2xl font-bold text-[#F5F1E7]">{notReady.length}</div>
