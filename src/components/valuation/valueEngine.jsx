...
export function normalizeValuationInputs(item, moduleKey) {
  ...
  if (moduleKey === 'winekeeper') {
    ...
    const manualUnit = toNum(item.manual_estimated_value);
    const manualTotal = item.manual_valuation_enabled && manualUnit > 0 ? manualUnit * qty : 0;
    const estimatedUnit = toNum(item.estimated_unit_value || item.estimated_value);
    const estimatedTotal = toNum(item.estimated_total_value) || (estimatedUnit > 0 ? estimatedUnit * qty : 0);
    const marketUnit = toNum(item.market_estimated_unit_value);
    const marketTotal = toNum(item.market_estimated_total_value) || (marketUnit > 0 ? marketUnit * qty : 0);
    const purchaseTotal = toNum(item.purchase_price);
    return {
      ...base,
      ...
      quantity: qty,
      manualValueOverride: manualTotal > 0 ? manualTotal : null,
      estimatedValue: estimatedTotal > 0 ? estimatedTotal : base.estimatedValue,
      marketValue: marketTotal > 0 ? marketTotal : base.marketValue,
      purchaseValue: purchaseTotal > 0 ? purchaseTotal : base.purchaseValue,
      confidence: item.valuation_confidence || item.market_valuation_confidence || base.confidence,
