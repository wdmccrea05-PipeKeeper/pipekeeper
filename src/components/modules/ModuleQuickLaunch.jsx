...
export default function ModuleQuickLaunch({ actions = [] }) {
  ...
  const cols = Math.min(actions.length, 5);
  const gridStyle = { gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))` };

  return (
    <div className="relative rounded-lg overflow-hidden p-6" style={{
      ...
      <h3 className="text-xs uppercase tracking-[0.14em] font-semibold mb-5 relative" style={{ color: 'rgba(180, 140, 75, 0.85)' }}>
        {t('quickActions.sectionTitle', 'Quick Actions')}
      </h3>
      <div className="grid gap-3.5 relative grid-cols-2 md:grid-cols-4" style={gridStyle}>
        {actions.map(({ key, Icon, iconImage, label, onClick }) => (
          ...
          >
            <div className="w-11 h-11 flex items-center justify-center overflow-hidden transition-transform group-hover:scale-110" style={{
              background: 'linear-gradient(135deg, rgba(100, 70, 45, 0.5), rgba(80, 55, 35, 0.6))',
              border: '1px solid rgba(120, 90, 65, 0.45)', borderRadius: '0.5rem', boxShadow: '0 3px 8px rgba(0,0,0,0.5), inset 0 1px 0 rgba(180, 140, 100, 0.2)',
            }}>
              {iconImage ? (
