...
export default function ModuleVisibilitySettings() {
  ...
  // All modules use the same toggle model — no module is locked or untouchable.
  // WhiskeyKeeper toggle is only shown to internal testers (it's not yet launched).
  const MODULE_CONFIG = [
    {
      id: 'pipekeeper',
    ...
    // WhiskeyKeeper: internal — only show to internal testers/admins
    ...(isTester ? [{
      id: 'whiskeykeeper',
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="font-semibold text-stone-100 text-base">{t('hub.activeModules', 'Active Modules')}</span>
      </div>
          ...
          return (
            <div
              ...
              className={`flex items-center justify-between gap-4 p-3 rounded-xl border transition-all ${
                enabled ? 'bg-stone-800/50 border-stone-700' : 'bg-stone-800/30 border-stone-700/50'
              }`}
            >
