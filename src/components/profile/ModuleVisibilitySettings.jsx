...
export default function ModuleVisibilitySettings() {
  ...
  const MODULE_CONFIG = [
    ...
    },
    {
      ...
      description: t('whiskeykeeper.description', 'Whiskey bottle collection, tasting notes, and inventory.'),
    ...
    },
    {
      ...
      description: 'Wine cellar management and bottle tracking.',
    ...
    },
    {
      ...
      description: 'Cigar collection curation and tasting.',
  ...
  async function handleToggle(moduleId, enabled) {
    if (!enabled) {
      ...
      const wouldHaveOneLeft = launchedModules.filter((m) => m.id !== moduleId && moduleStates[m.id] !== false).length >= 1;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="font-semibold text-stone-800 text-base">{t('hub.activeModules', 'Active Modules')}</span>
      </div>
      <p className="text-xs text-stone-500 mb-3">
        {t('profile.moduleVisibilityDescription', 'Choose which collection modules appear in your navigation and Hub. Hiding a module never deletes your data — you can re-enable it at any time.')}
      </p>
      ...
      <div className="space-y-3">
        {MODULE_CONFIG.map((mod) => {
          ...
          return (
            <div
              key={mod.id}
              className={`flex items-center justify-between gap-4 p-3 rounded-xl border transition-all ${enabled ? 'bg-stone-50 border-stone-200' : 'bg-stone-100/50 border-stone-200/60 opacity-60'}`}
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {mod.alcoholRelated && <Badge className="text-[10px] bg-amber-100 text-amber-700 border-0 px-1.5 py-0">Alcohol</Badge>}
                    {mod.canDisable === false && <Badge className="text-[10px] bg-blue-100 text-blue-700 border-0 px-1.5 py-0">Primary</Badge>}
                  </div>
      ...
      <p className="text-xs text-stone-400 mt-2">
        {t('profile.moduleVisibilityNote', 'Hiding a module removes it from navigation, Hub, and recommendations. Your records remain stored and fully intact.')}
      </p>
    </div>
  );
}
