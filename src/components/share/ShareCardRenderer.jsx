...
function Meta({ label, value }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: "#E0D8C8", fontWeight: 600, overflowWrap: "anywhere" }}>{value}</p>
    </div>
  );
}
    ...
    <div ref={ref} className="w-full max-w-sm mx-auto p-8" style={{ background: "linear-gradient(135deg, #2a1f18 0%, #1f1510 100%)", borderRadius: 16, border: "1px solid rgba(180, 140, 75, 0.25)", boxShadow: "0 8px 32px rgba(0, 0, 0, 0.4)" }}>
      ...
      <div className="space-y-4">
        <div className="border-b border-[rgba(180,140,75,0.15)] pb-4">
          <h2 className="text-2xl font-bold mb-1" style={{ color: "#FFFFFF", overflowWrap: "anywhere", lineHeight: 1.1 }}>{pipe?.name}</h2>
          {pipe?.maker ? <p className="text-sm font-semibold" style={{ color: "rgba(224,216,200,0.8)", overflowWrap: "anywhere" }}>{pipe.maker}</p> : null}
        </div>
...
export const TobaccoShareCard = React.forwardRef(({ tobacco }, ref) => {
  ...
  return (
    <div ref={ref} className="w-full max-w-sm mx-auto p-8" style={{ background: "linear-gradient(135deg, #2a1f18 0%, #1f1510 100%)", borderRadius: 16, border: "1px solid rgba(180, 140, 75, 0.25)", boxShadow: "0 8px 32px rgba(0, 0, 0, 0.4)" }}>
      ...
      <div className="space-y-4">
        <div className="border-b border-[rgba(180,140,75,0.15)] pb-4">
          <h2 className="text-2xl font-bold mb-1" style={{ color: "#FFFFFF", overflowWrap: "anywhere", lineHeight: 1.1 }}>{tobacco?.name}</h2>
          {tobacco?.manufacturer ? <p className="text-sm font-semibold" style={{ color: "rgba(224,216,200,0.8)", overflowWrap: "anywhere" }}>{tobacco.manufacturer}</p> : null}
        </div>
        <div className="grid grid-cols-2 gap-3 text-xs">
        ...
        {tobacco?.notes ? <div className="border-t border-[rgba(180,140,75,0.15)] pt-4"><p style={{ color: "rgba(224,216,200,0.8)", fontSize: 13, lineHeight: 1.5, overflowWrap: "anywhere" }}>{String(tobacco.notes).slice(0,150)}</p></div> : null}
      </div>
    </div>
  );
});