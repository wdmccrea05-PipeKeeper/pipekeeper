    ...
    <div ref={ref} className="w-full max-w-sm mx-auto p-8" style={{ background: "linear-gradient(135deg, #2a1f18 0%, #1f1510 100%)", borderRadius: 16, border: "1px solid rgba(180, 140, 75, 0.25)", boxShadow: "0 8px 32px rgba(0, 0, 0, 0.4)" }}>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-xs">
          <Meta label={t("pipes.shape")} value={pipe?.shape} />
          <Meta label={t("pipes.material")} value={pipe?.bowl_material} />
          <Meta label={t("pipes.bend")} value={pipe?.bend} />
          <Meta label={t("pipes.size")} value={pipe?.sizeClass} />
          <Meta label={t("pipes.origin")} value={pipe?.country_of_origin} />
          <Meta label={t("pipes.year")} value={pipe?.year_made} />
          <Meta label={t("pipes.condition")} value={pipe?.condition} />
          {pipe?.estimated_value ? <Meta label={t("share.value")} value={formatFromBase(pipe.estimated_value)} /> : null}
        </div>
  ...
  return (
    <div ref={ref} className="w-full max-w-sm mx-auto p-8" style={{ background: "linear-gradient(135deg, #2a1f18 0%, #1f1510 100%)", borderRadius: 16, border: "1px solid rgba(180, 140, 75, 0.25)", boxShadow: "0 8px 32px rgba(0, 0, 0, 0.4)" }}>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-xs">
          <Meta label={t("whiskey.type")} value={bottle?.type} />
          <Meta label={t("whiskey.region")} value={bottle?.region} />
          <Meta label={t("whiskey.age")} value={bottle?.age ? `${bottle.age} yr` : null} />
          <Meta label={t("whiskey.abv")} value={bottle?.abv ? `${bottle.abv}%` : null} />
          <Meta label={t("whiskey.vintage")} value={bottle?.vintage} />
          <Meta label={t("whiskey.country")} value={bottle?.country} />
          {bottle?.estimated_value ? <Meta label={t("share.value")} value={formatFromBase(bottle.estimated_value)} /> : null}
          {bottle?.rating ? <Meta label={t("common.rating")} value={`${bottle.rating}/5`} /> : null}
        </div>
  ...
  return (
    <div ref={ref} className="w-full max-w-sm mx-auto p-8" style={{ background: "linear-gradient(135deg, #2a1f18 0%, #1f1510 100%)", borderRadius: 16, border: "1px solid rgba(180, 140, 75, 0.25)", boxShadow: "0 8px 32px rgba(0, 0, 0, 0.4)" }}>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-xs">
          <Meta label={t("wine.style")} value={wine?.style} />
          <Meta label={t("wine.region")} value={wine?.region || wine?.appellation} />
          <Meta label={t("wine.vintage")} value={wine?.vintage} />
          <Meta label={t("wine.varietal")} value={wine?.varietal} />
          <Meta label={t("wine.country")} value={wine?.country || wine?.country_of_origin} />
          {wine?.estimated_value ? <Meta label={t("share.value")} value={formatFromBase(wine.estimated_value)} /> : null}
          {wine?.rating ? <Meta label={t("common.rating")} value={`${wine.rating}/5`} /> : null}
          {wine?.quantity ? <Meta label={t("share.inventory")} value={`${wine.quantity} bottle${wine.quantity === 1 ? '' : 's'}`} /> : null}
        </div>
  ...
  return (
    <div ref={ref} className="w-full max-w-sm mx-auto p-8" style={{ background: "linear-gradient(135deg, #2a1f18 0%, #1f1510 100%)", borderRadius: 16, border: "1px solid rgba(180, 140, 75, 0.25)", boxShadow: "0 8px 32px rgba(0, 0, 0, 0.4)" }}>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-xs">
          <Meta label={t("cigars.line")} value={cigar?.line} />
          <Meta label={t("cigars.vitola")} value={cigar?.vitola} />
          <Meta label={t("cigars.wrapper")} value={cigar?.wrapper} />
          <Meta label={t("cigars.origin")} value={cigar?.country_of_origin} />
          {cigar?.estimated_value ? <Meta label={t("share.value")} value={formatFromBase(cigar.estimated_value)} /> : null}
          {cigar?.rating ? <Meta label={t("common.rating")} value={`${cigar.rating}/5`} /> : null}
        </div>
  ...
  return (
    <div ref={ref} className="w-full max-w-sm mx-auto p-8" style={{ background: "linear-gradient(135deg, #2a1f18 0%, #1f1510 100%)", borderRadius: 16, border: "1px solid rgba(180, 140, 75, 0.25)", boxShadow: "0 8px 32px rgba(0, 0, 0, 0.4)" }}>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-xs">
          <Meta label={t("tobaccoExtended.blendType")} value={tobacco?.blend_type} />
          <Meta label={t("tobaccoExtended.cut")} value={tobacco?.cut} />
          <Meta label={t("tobaccoExtended.strength")} value={tobacco?.strength} />
          <Meta label={t("tobaccoExtended.roomNote")} value={tobacco?.room_note} />
          {tobacco?.estimated_value ? <Meta label={t("share.value")} value={formatFromBase(tobacco.estimated_value)} /> : null}
          {typeof tobacco?.total_quantity_oz === 'number' && tobacco.total_quantity_oz > 0 ? <Meta label={t("share.inventory")} value={`${tobacco.total_quantity_oz.toFixed(1)} oz`} /> : null}
        </div>
        {Array.isArray(tobacco?.tobacco_components) && tobacco.tobacco_components.length > 0 ? <div><p style={{ color: "rgba(180,140,75,0.6)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>{t("tobaccoExtended.tobaccoComponents")}</p><div className="flex flex-wrap gap-2">{tobacco.tobacco_components.slice(0,5).map((c)=><span key={c} className="px-2 py-1 rounded-full text-[11px]" style={{ background:"rgba(180,140,75,0.12)", border:"1px solid rgba(180,140,75,0.18)", color:"#E0D8C8", overflowWrap:"break-word", wordBreak:"normal" }}>{c}</span>)}</div></div> : null}
