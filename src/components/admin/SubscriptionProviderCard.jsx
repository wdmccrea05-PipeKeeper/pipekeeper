...
export default function SubscriptionProviderCard({ me }) {
  ...
  const openStripePortal = () => {
    ...
    if (url) {
      ...
      setError(t('admin.unableToOpenStripePortal'));
    }
  };
  ...
  const continueApplePurchase = () => {
    if (!doubleBillAck) {
      setError(t('admin.doubleBillingAckText'));
      return;
    }
    setError(null);
      ...
      <Card className="border-[#A35C5C]/30">
        <CardContent className="p-6 space-y-3">
          <Alert variant="destructive">
            ...
            <AlertDescription>
              {fetchError?.message || summary?.error || t('admin.providerCardFailed')}
            </AlertDescription>
          </Alert>
    ...
    <Card className="border-[#A35C5C]/30">
      <CardContent className="p-6 space-y-4">
        <div className="flex items-start justify-between">
          <div>
            <h3 className="text-lg font-semibold text-[#E0D8C8]">{t('admin.subscriptionManagement')}</h3>
        ...
        {provider === "apple" && ios && (
          <Button onClick={manageApple} className="w-full">
            ...
            {t('admin.manageAppleSubscriptions')}
          </Button>
        )}
              ...
              <div className="border border-white/10 rounded-lg p-4 space-y-3 bg-white/5">
                <div className="flex items-center justify-between">
                  <div className="text-sm font-semibold text-[#E0D8C8]">
                    {t('admin.switchToAppleBillingTitle')}
                  </div>
                  ...
                  <div className="space-y-3 text-sm">
                    <Alert>
                      ...
                      <AlertDescription className="text-xs">
                        {t('admin.appleDoubleBillingAlert')}
                      </AlertDescription>
                    </Alert>

                    <div className="flex items-center gap-2">
                      <label className="text-sm text-[#E0D8C8]">{t('admin.tierLabel')}</label>
                      <select
                    ...
                    <div className="flex gap-2">
                      <Button 
                        ...
                        className="flex-1"
                      >
                        {t('admin.openWebManagement')}
                      </Button>
                      <Button 
                        onClick={continueApplePurchase}
                        className="flex-1"
                      >
                        {t('admin.continueToApplePurchase')}
                      </Button>
                    </div>
                  </div>
                )}
        ...
        {provider === "none" && (
          <div className="space-y-3">
            <p className="text-sm text-[#E0D8C8]/70">{t('admin.noActiveSubscription')}</p>
            {ios ? (
              <div className="flex gap-2">
                <Button onClick={() => startApplePurchaseFlow("premium")} className="flex-1">
                  {t('admin.buyPremiumApple')}
                </Button>
