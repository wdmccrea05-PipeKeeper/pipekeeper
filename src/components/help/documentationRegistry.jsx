...
export const HELP_ARTICLES = [
  ...
  // ─── PIPE CLUB ───────────────────────────────────────────────────────────────
  {
    id: 'pipe-club-overview',
    title: 'Pipe Club',
    module: 'pipekeeper',
    category: 'getting-started',
    summary: 'Find the best pipe from what you brought for any club tobacco — even blends you don\'t own.',
    body: 'Pipe Club is a session-based feature for pipe club meetings and gatherings. When someone brings a tobacco you may not own, open Pipe Club from the main navigation and tap Start Session. The wizard walks you through five steps: (1) Session — enter the date, club name, and location. (2) Pipes — multi-select the pipes you physically brought with you; for system pipes with interchangeable bowls, choose the specific bowl you have on hand. (3) Blend — identify the club tobacco from your collection, your wishlist, or enter a new/unowned blend using Quick Lookup or AI identification. (4) Recommendation — the canonical pairing engine scores only the pipes you brought and shows the best match with a confidence tier and explanation. (5) Log — record which pipe you actually used, rate the tobacco and the pairing, note whether you\'d smoke it again, and optionally add the blend to your wishlist or mark it Not For Me. Sessions are saved to your history and appear in the Session History calendar.',
    keywords: ['pipe club', 'club session', 'meeting', 'gathering', 'session wizard', 'present pipes', 'club blend', 'recommendation', 'quick lookup', 'identify blend', 'bowl variant', 'disposition', 'wishlist', 'not for me'],
    questions: [
      'what is pipe club',
      'how do i use pipe club',
      'how do i start a pipe club session',
      'how do i find the best pipe for a club tobacco',
      'what is a pipe club session',
      'how do i use pipe club at a meeting',
      'how do i score pipes i brought',
    ],
    synonyms: ['club meeting', 'pipe meeting', 'club session', 'group smoke', 'session wizard'],
    relatedArticles: ['pipe-club-blend-entry', 'pipe-club-recommendation', 'pipe-club-logging', 'pipekeeper-ai-pairings'],
  },
  {
    id: 'pipe-club-blend-entry',
    title: 'Identifying a Club Blend',
    module: 'pipekeeper',
    category: 'how-to',
    summary: 'Select the club tobacco from your collection, wishlist, or enter a new blend with AI-assisted metadata.',
    body: 'On the Blend step of a Pipe Club session, choose from three sources: My Collection — search and select a blend you already own. Wishlist — select a blend from your Want List. New / Not Owned — enter a blend you don\'t own. For new blends, use Quick Lookup to search the entire global tobacco database (all blends known to the system, not just yours) and auto-fill metadata by selecting a match. If Quick Lookup doesn\'t find it, enter the manufacturer and blend name, then tap Identify Blend (AI) to auto-fill blend type, family, aromatic status, intensity, tobacco components, cut, strength, casing, and topping. The AI returns null for fields it can\'t confidently determine — fill those in manually. Accurate metadata ensures the pairing engine produces a reliable recommendation.',
    keywords: ['club blend', 'blend entry', 'quick lookup', 'identify blend', 'ai identify', 'new blend', 'unowned blend', 'wishlist blend', 'blend metadata', 'blend type', 'aromatic', 'cut', 'strength'],
    questions: [
      'how do i identify the club blend',
      'how do i enter a new blend in pipe club',
      'what is quick lookup',
      'how do i use quick lookup',
      'how do i identify a blend with ai',
      'what does identify blend do',
      'how do i select a blend from my collection',
      'how do i select a wishlist blend',
    ],
    synonyms: ['blend selection', 'blend lookup', 'blend identification', 'tobacco entry'],
    relatedArticles: ['pipe-club-overview', 'pipe-club-recommendation'],
  },
  {
    id: 'pipe-club-recommendation',
    title: 'Pipe Club Recommendation',
    module: 'pipekeeper',
    category: 'features',
    summary: 'The canonical pairing engine scores only the pipes you brought and recommends the best match for the club blend.',
    body: 'After selecting your present pipes and identifying the club blend, the Recommendation step shows the best-matched pipe from the ones you have with you. The engine uses the same multi-dimensional scoring as the full Pairing Matrix — pipe specialization, chamber geometry, tobacco cut, blend composition, aromatic compatibility, bowl material, and smoking character — but narrowed to only the pipes you selected as present. For system pipes where you chose a specific bowl, the engine scores that bowl\'s geometry and material. The recommendation includes a confidence tier (High, Medium, or Low) and a plain-language explanation of why the pairing works. A runner-up alternative pipe is also shown when available.',
    keywords: ['recommendation', 'pairing', 'canonical engine', 'scoring', 'best pipe', 'confidence', 'alternative', 'present pipes', 'bowl variant scoring'],
    questions: [
      'how does the pipe club recommendation work',
      'how does pipe club score pipes',
      'what does the confidence tier mean',
      'why is a different pipe recommended',
      'what is the alternative pipe',
      'does pipe club use the full pairing matrix',
    ],
    synonyms: ['pairing recommendation', 'best pipe match', 'club recommendation'],
    relatedArticles: ['pipe-club-overview', 'pipe-club-blend-entry', 'pipekeeper-ai-pairings'],
  },
  {
    id: 'pipe-club-logging',
    title: 'Logging a Pipe Club Session',
    module: 'pipekeeper',
    category: 'how-to',
    summary: 'Record which pipe you actually used, rate the tobacco and pairing, and set a disposition for unowned blends.',
    body: 'After the session, the Log step lets you capture the real-world outcome. Select which pipe you actually smoked — this may differ from the recommendation if you chose to use a different pipe. Rate the overall tobacco (1–5 stars) and the pipe + tobacco pairing (1–5 stars). Indicate whether you would smoke this combination again (Yes, No, or Not sure). Add post-session notes about how it smoked. For blends not from your collection, choose a disposition: Add to Wishlist adds the blend to your Want List so you can acquire it later. Not For Me marks the blend to exclude it from future AI recommendations. The session is saved to your Pipe Club history and appears in the Session History calendar alongside your regular smoking logs.',
    keywords: ['log session', 'pipe club log', 'rating', 'pairing rating', 'would smoke again', 'post-session notes', 'disposition', 'wishlist', 'not for me', 'session history'],
    questions: [
      'how do i log a pipe club session',
      'how do i rate a club tobacco',
      'how do i add a club blend to my wishlist',
      'how do i mark a blend not for me',
      'where are my pipe club sessions saved',
      'how do i record which pipe i used',
    ],
    synonyms: ['session log', 'club session record', 'post-session', 'disposition'],
    relatedArticles: ['pipe-club-overview', 'pipe-club-recommendation', 'log-smoking-session', 'want-list'],
  },
...
export const TOPIC_SHORTCUTS = {
  ...
  'pipe club': 'pipe-club-overview',
  'club session': 'pipe-club-overview',
  'club meeting': 'pipe-club-overview',
  'club blend': 'pipe-club-blend-entry',
  'quick lookup': 'pipe-club-blend-entry',
  'identify blend': 'pipe-club-blend-entry',
  'club recommendation': 'pipe-club-recommendation',
  'present pipes': 'pipe-club-overview',
...
export function getContextualHelp(screenName) {
  const map = {
    ...
    'humidor': 'cigar-humidor',
    'pipeclub': 'pipe-club-overview',
    'pipe club': 'pipe-club-overview',
    'club': 'pipe-club-overview',
  };
