...
function buildCanonicalUsers(users, contracts, activityIndex, eventIndex, entitlements, referralAccess, usersByEmail, usersById, now) {
  ...
  for (const user of users) {
    ...
    userRecords.push({
      ...
      first_login_at: parseMetricDate(user.first_login_at || user.first_login_date || user.first_sign_in_at || null),
      last_login_at: parseMetricDate(user.last_login_at || user.last_login_date || user.last_sign_in_at || null),
      verification_email_at: parseMetricDate(user.email_verified_at || user.verification_email_at || null),
      verification_phone_at: parseMetricDate(user.phone_verified_at || user.verification_phone_at || null),
      acquisition_source: user.acquisition_source || user.source || null,
      referral_source: user.referral_source || user.referrer || null,
      utm_source: user.utm_source || null,
      utm_medium: user.utm_medium || null,
      utm_campaign: user.utm_campaign || null,
      utm_term: user.utm_term || null,
      utm_content: user.utm_content || null,
      platform_history: Array.isArray(user.platform_history) ? user.platform_history : [],
      device_history: Array.isArray(user.device_history) ? user.device_history : [],
  ...
  for (const sid of syntheticUserIds) {
    ...
    userRecords.push({
      ...
      first_login_at: null,
      last_login_at: null,
      verification_email_at: null,
      verification_phone_at: null,
...
Deno.serve(async (req) => {
  try {
    ...
    const auditUsers = userRecords.map((r) => ({
      ...
      first_login_at: r.first_login_at?.toISOString() || null,
      last_login_at: r.last_login_at?.toISOString() || null,
      verification_email_at: r.verification_email_at?.toISOString() || null,
      verification_phone_at: r.verification_phone_at?.toISOString() || null,
    ...
    return Response.json({
      ...
      lifecycleDataGaps: {
        missing_field_reasons: [
          'utm_source/utm_medium/utm_campaign not present on all historical users',
          'verification milestones are sparse for legacy records',
          'platform_history and device_history currently default empty when unavailable',
        ],
        required_fields: [
          'acquisition_source',
          'referral_source',
