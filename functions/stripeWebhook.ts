    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (session.mode === 'subscription') {
          // Retrieve full subscription object for accurate dates
          ...
          // Update user subscription level and customer ID
      ...
      case 'customer.subscription.updated': {
        ...
        if (existingSubs.length > 0) {
          ...
          // Create if it doesn't exist (covers subscriptions created outside checkout)
      ...
      case 'invoice.payment_failed': {
        ...
        if (invoice.subscription) {
          ...
          // Mark user as having payment issues
          await base44.asServiceRole.auth.updateUser(userEmail, {
            subscription_level: 'past_due',
          });
        }
        break;
      }
