...
Deno.serve(async (req: Request) => {
  try {
    ...
    console.log(`[adminReconcileEntitlements] Processing: ${targetEmail}`);
    ...
    if (!userEntity) {
      console.log(`[adminReconcileEntitlements] User not found: ${targetEmail}`);
      return Response.json({
        ok: false,
    ...
    if (result.changed) {
      ...
      console.log(`[adminReconcileEntitlements] Updated user ${targetEmail}: ${before.subscription_tier} → ${finalTier}`);
    } else {
      console.log(`[adminReconcileEntitlements] No changes needed for ${targetEmail}`);
    }
    ...
    console.error("[adminReconcileEntitlements] Fatal error:", error);
    return Response.json({
      ok: false,
